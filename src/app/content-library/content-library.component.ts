...
export class ContentLibraryComponent implements OnInit {
  ...
  getContentLibraryArticles() {
    ...
    this.apiService.getCategories().subscribe(data => {
      ...
      // this.loading = true;
      const defaultCategory = this.categories.filter(
        category => category.is_default
      );
      this.apiService.getContentLibraryArticles(defaultCategory[0]).subscribe(
        data => {
  ...
  getArticlesBasedOnCategory(guid, name) {
    ...
    this.apiService.getContentLibraryArticles(currentCategory[0]).subscribe(
      data => {
        this.articles = this._sanitizeImageUrls(data);
        this.loading = false;
  ...
  ngOnChanges(changes: { [property: string]: SimpleChange }) {
    ...
    if (change.currentValue == "Show") {
      if (this.articles == undefined) this.getContentLibraryArticles();
    }
  }
}
